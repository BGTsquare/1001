...
export default async function ReadPage({ params }: ReadPageProps) {
  ...
  if (authError || !user) {
    redirect('/auth/login?redirect=/books/' + params.id + '/read')
  }
  ...
  if (!book.is_free) {
    ...
    if (!ownershipResult.success || !ownershipResult.data?.owned) {
      redirect('/books/' + params.id + '?error=not-owned')
    }
  }

  // Get library item for progress tracking
  const libraryResult = await libraryService.getLibraryItem(user.id, params.id)
  
  if (!libraryResult.success || !libraryResult.data) {
    // If book is free but not in library, add it
    if (book.is_free) {
      const addResult = await libraryService.addBookToLibrary(user.id, params.id, 'owned')
      if (addResult.success && addResult.data) {
        // Redirect to reload with library item
        redirect('/books/' + params.id + '/read')
      }
    }
    
    redirect('/books/' + params.id + '?error=library-error')
  }
